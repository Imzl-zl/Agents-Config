---
description: "Initialize or normalize project collaboration rules, CLAUDE.md, tools.md, and memory files"
...
- `CLAUDE.md`：项目唯一 canonical 规则文件
- `AGENTS.md`：仅在仓库里已存在时改写为兼容入口，默认不新建
...
- 默认目标始终是：`CLAUDE.md` 作为唯一主规则文件
- `CLAUDE.md` 采用“最小骨架 + 按仓库信号追加独立小节”的生成策略：
  - 最小骨架至少包含：
    - `0. 实施基线`
    - `1. 作用`
    - `2. Source of Truth`
  ...
  - 如果仓库已有足够强的规则/文档/代码信号，可额外拆出独立小节：
    ...
    - `文档同步规则`
    - `禁止事项`
- `AGENTS.md` 如果仓库里原本存在，则在备份后改写为 `@CLAUDE.md`
...
10. 生成或归一结束后，只允许保留一个 substantive 规则文件：`CLAUDE.md`。
11. 如果已有 `AGENTS.md`，归一后它只能作为 `@CLAUDE.md` 的兼容入口，不能继续保留为第二份独立 substantive 规则文件。
12. 如果已有 `CLAUDE.md` / `AGENTS.md` / `tools.md` / `memory.md` 只是空壳、占位或薄内容文件，必须在备份后直接重建，不能因为“文件存在”就跳过。
13. 结构、分层、依赖方向、关键入口、验证命令属于强制扫描填充项；不要直接生成空泛模板。
14. 文档优先级清单、项目特定禁止事项、模块特别规则只在仓库已有强信号时生成；不要臆造不存在的文档名、目录职责或禁令。
15. `CLAUDE.md` / `tools.md` / `memory.md` 的每个 section 优先写仓库事实；若确实提取不到，单个 section 最多写一条 `[待补充]`，不要堆重复占位符。
...
- 只能从仓库内**显式存在**的信号提取标准验证命令，例如：
  ...
  - CI 配置中的真实校验命令
  - `README`、`docs`、现有 `CLAUDE.md` / `AGENTS.md` / `tools.md` 中明确写出的验证命令
...
只要 `CLAUDE.md` 和 `AGENTS.md` 都不是 substantive，就进入“全新初始化模式”。

处理规则：

- 如果两者都不存在：直接按模板创建 `CLAUDE.md`
- 如果其中任一存在但属于 non-substantive：先备份已有文件，再按模板创建 `CLAUDE.md`

#### 情况 B：项目中已经有 substantive 规则文件

只要 `CLAUDE.md` 或 `AGENTS.md` 至少有一个是 substantive，就进入“归一优化模式”。

此时不再询问，直接进入“归一优化模式”：

- 先备份已有文件到项目根 `init-project.backup/[timestamp]/`
- 再按当前标准生成新的 canonical `CLAUDE.md`
- 如果已有 `AGENTS.md`，则改写为 `@CLAUDE.md`
...
`CLAUDE.md` 必须由以下三类来源合成，而不是只靠单一模板或单一来源：
...
3. **用户已有规则**
   - 只要仓库里已有 substantive `CLAUDE.md` / `AGENTS.md`，就必须提取其中仍有效的项目规则
   - 重点继承：模块边界、真值源规则、项目特定限制、已有文档优先级、关键禁止事项
   - 这部分不能因为重建模板而丢失

可以把它理解成：

`CLAUDE.md = 固定协作骨架 + 当前仓库事实 + 用户已有有效规则`
...
4. **近期活动线索**
   ...
   - 不进入 `CLAUDE.md` 的正式规则区
### 3c. 归一目标

归一后应得到：

- 一个 substantive `CLAUDE.md`
- 可选的兼容入口 `AGENTS.md`
...
- 先备份
- 提取并保留其现有有效规则
- 按当前标准重写为新的 canonical `CLAUDE.md`
...
- 先备份
- 从 `AGENTS.md` 提取所有仍有效的项目规则，生成新的 canonical `CLAUDE.md`
- `AGENTS.md` 直接改写为 `@CLAUDE.md`

#### `CLAUDE.md` substantive + `AGENTS.md` pointer

- 先备份已有文件
- 以 `CLAUDE.md` 作为用户已有规则的提取主源
- 提取现有有效规则，按当前标准重写为新的 canonical `CLAUDE.md`
- `AGENTS.md` 重写为 `@CLAUDE.md`

#### `CLAUDE.md` pointer + `AGENTS.md` substantive

- 先备份已有文件
- 以 `AGENTS.md` 作为用户已有规则的提取主源生成新的 canonical `CLAUDE.md`
- 再把 `AGENTS.md` 改写为 `@CLAUDE.md`

#### `CLAUDE.md` 和 `AGENTS.md` 都 substantive
...
- 冲突项优先参考正式设计和当前代码
- 仍无法确认的写 `[待人工确认]`
- 合并产出一个 canonical `CLAUDE.md`
- `AGENTS.md` 改为 `@CLAUDE.md`
...
- 如果现有规则已经很强，不要硬压回一个薄骨架；可以保留为更多独立小节，只要最终 `CLAUDE.md` 仍然短、小、硬。

---

## 第四步：生成或补强 `CLAUDE.md`

`CLAUDE.md` 是最终 canonical 规则文件，必须至少覆盖以下内容。
...
- 近期状态进 `memory.md`
- 长期稳定协作知识进 `tools.md`
- 项目特定解释性长文回到 `docs/` 或正式文档，不堆进 `CLAUDE.md`
...
- **Changes**：生成/补强 `CLAUDE.md`、`tools.md`、`memory.md`、`memory/archive/YYYY-MM.md`；若仓库原有 `AGENTS.md`，则将其改写为 `@CLAUDE.md`。
...
- `CLAUDE.md` 不是空骨架，至少已经写入基线、结构/依赖/入口、协作文件生命周期规则
- `tools.md` 至少包含真实命令、路径、模式、坑点中的高信号内容，不是纯占位
- `memory.md` 的“已完成能力”不是 commit 标题堆砌，“最近活跃窗口”只覆盖最近 2~3 天
- 如果仓库存在 `AGENTS.md`，其最终内容只剩 `@CLAUDE.md`
- 所有被重写的文件都已备份到项目根 `init-project.backup/[timestamp]/`
- 如果仓库原本存在 substantive `CLAUDE.md` 或 `AGENTS.md`，必须额外确认：
  - 其中仍有效的模块边界、真值源规则、项目特定禁令、文档优先级、验证命令已经迁移进新的 `CLAUDE.md`，而不是静默丢失
