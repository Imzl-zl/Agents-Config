...
5. 每次等待用户前更新 state，并输出压缩 state 块（见"跨会话设计检查点"）；不要等到访谈结束才第一次形成 handoff。
...
每次新的 `/dev-workflow` invocation 在重新分类意图前做恢复扫描：用户给出 checkpoint、项目根存在 `.dev-workflow/config.yaml`、默认 `.tasks/` 或后备 `.dev-workflow/tasks/` 带有 `.dev-workflow-root` marker——任一信号成立即读取 `RECOVERY.md` 并按其流程 hydrate。同一 task 的真值优先序固定为：`DESIGN.md` → PROGRESS 中 active `Design Drift` → 非终态 CSV tracker。
