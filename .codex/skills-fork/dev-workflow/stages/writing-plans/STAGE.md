...
这是 `/dev-workflow` 的 supporting stage，不是独立 skill。它只在编排器已设置 `durable-plan` 或 `epic-plan` 后物化执行工件；完成后返回 plan delta，由当前编排 frame 连续进入 execution。

所有 `.tasks/...` 路径都是 `WorkflowState.task_root` 默认值的示例；项目选择其他专用根时统一替换，禁止同时写两个根。

物化前必须由编排器确认 `<task_root>/.dev-workflow-root` ownership marker；本 stage 不在未标记的非空目录中创建工件。
...
`dev-workflow` 通常会把 Transient 直接交给执行阶段；只有需要记录 shape 选择本身时，计划阶段才记录一个不落盘的 Transient 结论。
...
CSV 的 `goal` 是对模型目标的稳定引用，不是另一份长篇计划。
...
1,P1,实现登录 API,"只改 /api/auth，不改前端",src/api/auth.ts,"symbol:UserService.login;route:POST /api/auth/login",,TODO,curl 返回 200,,JWT 24h
```

字段：

- `id`：稳定任务编号，创建后不重排。
- `plan_ref`：可选的会话 plan 对应项，如 `P1`。
- `goal`：一个可独立验收的目标。
- `boundary`：允许和禁止修改的范围。
...
9. 把全部候选提交到最终路径并重新读取验证；将删除 DESIGN 作为最后一次真值写操作。删除失败则保持 DESIGN 为真并返回 blocked；删除成功后不得再依赖 post-delete 写入才能执行。
10. 返回 `StageStatus: plan-ready`、Shape、Truth、Artifacts 和第一条 dependency-ready task；不 invoke 入口或 execution stage。
