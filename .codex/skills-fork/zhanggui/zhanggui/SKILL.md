...
每次新的 `/zhanggui` invocation 在重新分类意图前做恢复扫描：用户给出 checkpoint、项目根存在 `.zhanggui/config.yaml`、默认 `.tasks/` 或后备 `.zhanggui/tasks/` 带有 `.zhanggui-root` marker——任一信号成立即读取 `RECOVERY.md` 并按其流程 hydrate。同一 task 的真值优先序固定为：`DESIGN.md` → PROGRESS 中 active `Design Drift` → 非终态 CSV tracker。
